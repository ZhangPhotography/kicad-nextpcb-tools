...
from kicad_nextpcb_new.nextpcb_tools_gui.foot_print_list import FootPrintList
from kicad_nextpcb_new.nextpcb_tools_gui.ui_match_part_panel.match_part_view import MatchPartView
...
BOARD_LOADED = load_board_manager()
...
class NextPCBTools(wx.Dialog):
    ...
    def enable_toolbar_buttons(self, state):
        ...
        for button in (
            ...
            self.match_part_view.down_toolbar.EnableTool(button, state)
    ...
    def get_part_details(self, e):
        ...
        mpn = self.footprint_list.GetTextValue(row, 4)
        if not mpn:
            self.assigned_part_view.get_part_data( 0 ) 
        else:
